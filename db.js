...
exports.getRecipe = function(recipeID, callback) {
	...
	var sql = 'SELECT * FROM (SELECT * FROM ingredients INNER JOIN metadata ON ingredients.recipeID = metadata.recipeID) AS ingredients_metadata INNER JOIN steps ON steps.recipeID = ingredients_metadata.recipeID WHERE steps.recipeID = ?';
	var data;
